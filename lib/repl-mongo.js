...
module.exports = function(Vantage, options) 
{
		...
		.action(function(args, cb) {
			try {
				mongo.db[args.collection].find({}).exec(function(err,objs) { 
					Vantage.log( Util.inspect(objs, {showHidden: true, colors: true}) );
					cb();
				});
			}
			catch (e)
			{
				Vantage.log('"' + args.collection + '" is not a collection.');
				cb();
			}
		});
	...
	VantageProxy
		...
		.init(function(args, cb) {
			...
			Vantage.session._modeDelimiter = (mq.qDelim.trim() + mq.qModeFn+ '{}' + ':');

			Vantage.log(' Enter Mongo filter object\'s content ( stuff inbetween {} ).\n'
						+ 'FILTER MODE (.f)\n'
						+ (parseModeEval
							? " * -> e.g.  name: {$in: ['test2']}\n"
							: ' * -> e.g.  "name": {"$in": ["test2"]}\n')
		...
		.action(function(command, cb) {
			...
			Vantage.session._modeDelimiter = (mq.qDelim.trim() +  mq.qModeFn + (x.join('')+'') + ':');

			if (!mongo.db[mq.qCollection])
			{
				Vantage.log('"' + args.collection + '" is not a collection anymore.');
				cb();
				return;
			}
			...
			try
			{
				...
				if ( (m = mongo.db[mq.qCollection][(qModeOverwrite||mq.qModeFn)](execObj)) )
				{
					...
					// the callback
					m.exec(function(err,objs) { Vantage.log( Util.inspect(objs, {showHidden: true, colors: true}) ); cb(); });
				}
				else
				{
					Vantage.log('done.');
	...
	VantageProxy
		...
		.init(function(args, cb) {
			Vantage.log(' Enter the .MongoCommand and param object\'s content ( stuff inbetween {} ).\n'
						+ (parseModeEval
